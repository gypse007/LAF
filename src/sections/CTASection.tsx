...
export default function CTASection() {
  ...
  return (
    ...
    >
      ...
      <div
        ref={glowRef}
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] sm:w-[600px] sm:h-[600px] bg-[#C9A962]/20 rounded-full blur-[80px] sm:blur-[150px]"
      />
      ...
      <div className="absolute inset-0 opacity-10">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className="absolute top-1/2 left-1/2 w-px h-[200%] bg-gradient-to-b from-[#C9A962] to-transparent origin-top"
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
        <div ref={contentRef} className="text-center">
          {/* Badge */}
          <span className="inline-block px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-[#C9A962]/30 text-[#C9A962] text-xs sm:text-sm tracking-widest uppercase mb-6 sm:mb-8">
            Start Your Project
          </span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-10 sm:mb-16">
            <a
              href="https://wa.me/918019818999?text=Hi%20Lakshmi%20Art%20Fixers%20Team%2C%0AI%E2%80%99m%20interested%20in%20commercial%20wall%20branding%20for%20my%20space.%0A%0A%F0%9F%93%8D%20Location%3A%0A%F0%9F%8F%A2%20Business%20Type%3A%0A%F0%9F%93%90%20Approx%20Wall%20Area%3A%0A%F0%9F%93%85%20Expected%20Completion%20Date%3A"
              target="_blank"
              rel="noopener noreferrer"
              className="group relative inline-flex items-center gap-2 sm:gap-3 px-8 sm:px-10 py-4 sm:py-5 rounded-full bg-[#C9A962] text-black font-bold text-base sm:text-lg overflow-hidden transition-all duration-500 hover:shadow-[0_0_40px_rgba(201,169,98,0.5)]"
            >
            ...
            <a
              href="#work"
              className="group inline-flex items-center gap-2 sm:gap-3 px-6 sm:px-8 py-4 sm:py-5 rounded-full border border-white/20 text-white font-medium hover:border-[#C9A962]/50 hover:text-[#C9A962] transition-all duration-300"
            >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
            <a
              href="tel:8019818999"
              className="flex items-center gap-2 sm:gap-3 text-white/60 hover:text-[#C9A962] transition-colors"
            >
